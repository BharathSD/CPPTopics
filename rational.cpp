...
Rational Rational::operator + (const Rational& rhs) const {
    return Rational((n * rhs.d) + (d * rhs.n), d * rhs.d).reduce();
}
...
Rational Rational::operator * (const Rational& rhs) const {
    return Rational(n * rhs.n, d * rhs.d).reduce();
}

Rational Rational::operator / (const Rational& rhs) const {
    return Rational(n * rhs.d, d * rhs.n).reduce();
}
...
Rational operator + (const Rational& lhs, const Rational& rhs) {
    ...
    lhs.denominator() * rhs.denominator()).reduce();
}
...
Rational operator * (const Rational& lhs, const Rational& rhs) {
    return Rational(lhs.numerator() * rhs.numerator(), lhs.denominator() * rhs.denominator()).reduce();
}

Rational operator / (const Rational& lhs, const Rational& rhs) {
    return Rational(lhs.numerator() * rhs.denominator(), lhs.denominator() * rhs.numerator()).reduce();
}
...
void rationalPrinter(const std::string rationalData){
    print("Rational value is: {}\n", rationalData);
}
...
int main() {
    ...
    // lets see what happens if we do 1 + a
    print("1 + a = {}\n", 1 + a);
#endif

    std::string ptrstr{"The rational number is : "};
    ptrstr += b;
    print("{}\n", ptrstr);
